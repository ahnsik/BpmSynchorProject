...
public class WavPlay extends Thread {
	...
	public void run() {
	    ...
	    int nBytesWritten = 0;
	    int playing_sec = 0;
	    int sample_rate = (wavBuffer[24]&0xFF)+((wavBuffer[25]&0xFF)<<8);		//( ((long)(Header[25]&0xFF)<<8)+Header[24]) );	// = 4����Ʈ little endian
...
//		int block_align = ((wavBuffer[33]&0xFF)<<8)+(wavBuffer[32]&0xFF);		// 1�� Sample �� byte ��. (= num_bytes_of_sample * num_of_channel )		//   ( num_of_channel * num_bits_of_sample/8 );			// num_of_channel
